...
void main() {
  ...
  // Water keeps alpha=1 in this pack, so stacked water volumes can skip the
  // "behind translucents" fog path entirely unless we opt in explicitly.
  bool useBehindTranslucentFog = translucentCheck || (isWater && VLBehindTranslucents.a < 0.9995);
  // ensure that bloomy fog mask in this VLBehindTranslucents.a does not darken outside of glass areas.
  if(useBehindTranslucentFog) color.rgb = color.rgb * VLBehindTranslucents.a + VLBehindTranslucents.rgb;
